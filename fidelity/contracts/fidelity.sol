...
/**
 * @title Fidelity Token
 * Author : https://github.com/xas
 * Version : 0.4.0 Proof Of Concept edition
*/
contract Fidelity is ERC20 {
    ...
    *
    * The order should be statistically better than the inverse
    * majority of people will wait 72 hours => tx cheaper
    ...
    */
    function computeBurn(address _origin) internal view returns (uint8) {
        if (block.timestamp > transferAge[_origin] + 72 * 1 hours) {
            return 0;
        }
        if (block.timestamp > transferAge[_origin] + 48 * 1 hours) {
            return 1;
        }
        if (block.timestamp > transferAge[_origin] + 24 * 1 hours) {
            return 10;
        }
        if (block.timestamp > transferAge[_origin] + 12 * 1 hours) {
            return 50;
        }
        if (block.timestamp > transferAge[_origin] + 6 * 1 hours) {
            return 100;
        }
        return 250;
    }
